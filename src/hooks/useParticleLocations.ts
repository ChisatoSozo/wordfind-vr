...
export const useParticleLocations = (meshRef: RefObject<Mesh>, num = 1000, width = 1, height = 1, offset = new Vector3(0, 0, 0), onlyBlack = false, textureReady = true) => {
    ...
    useEffect(() => {
        ...
        while (particles.length < num) {
            ...
            if (pixels[(y * texture.getSize().width + x) * 4 + 3] !== 0 && (!onlyBlack || pixels[(y * texture.getSize().width + x) * 4] < 0.5)) {
                ...
                const pos = new Vector3(width * xPerc - width / 2, height * yPerc - height / 2, 0).add(letterMesh.position).add(offset);
                particles.push(pos);
            }
        }
